...
- Browserbase = Red `#FF4500` · Stagehand = Green `#00C851` · Director = Blue `#00B0FF`
...
**Goal:** Inspire more people to take on ambitious automation on the web. All three brands (Browserbase, Stagehand, Director) share five voice attributes; each leans into one as its expressive signature.

### Five voice attributes

| Trait | Signature brand | Can be | Should NOT be |
|---|---|---|---|
| **Open** | All | Warm, welcoming, curious, approachable | Arrogant, snarky, passive, over-accommodating |
| **Grounded** | All | Steady, calm, clear, credible | Hype-y, smug, boring, tedious |
| **Insightful** | **Browserbase** | Perceptive, bold (when making a claim), direct, purposeful | Clever, cryptic, sarcastic, dramatic |
| **Inventive** | **Stagehand** | Practical, clear, methodical, resourceful | Chaotic, "mad scientist," dramatic, complicated |
| **Empowering** | **Director** | Supportive, confident, encouraging, clarifying | Hype, overly technical, confusing, vague |
...
**Insightful (Browserbase signature)** — clear thinking in complex situations. Helps people understand what's actually happening. Use when explaining the "why," not just the "what."
**Inventive (Stagehand signature)** — practical problem solving. The voice of someone who makes things simpler, not more complicated. Use when explaining new solutions.
**Empowering (Director signature)** — gives people a sense of capability. Makes the next step feel obvious. Use to reduce hesitation and create momentum.

### Tone

Overall tone across all brands: **conversational and confident.**
...
|---|---|---|---|
| `--bb-red` | `#FF4500` | 1655 C | Browserbase signature. CTAs, key highlights, logomark. One per viewport. |
| `--bb-black` | `#000000` | Black 6 C | Primary text on light, primary background in dark mode. |
| `--bb-white` | `#FFFFFF` | — | Primary background in light mode, text on dark/red. |
| `--bb-grey` | `#C5D3E8` | 657 C | Core grey. Interface/layout backgrounds, charts, diagrams. |
...
| Token | Hex | Pantone | Association |
|---|---|---|---|
| `--bb-stagehand-green` | `#00C851` | 2420 C | Stagehand product signature |
| `--bb-lime-green` | `#C4D600` | 382 C | Supporting accent |
| `--bb-director-blue` | `#00B0FF` | 306 C | Director product signature |
| `--bb-blue` | `#0000CD` | Blue 072 C | Supporting accent |
| `--bb-magenta` | `#FF00FF` | Real Purple C | Expressive accent — specialty ink for print |
| `--bb-pink` | `#FFC0CB` | 182 C | Supporting accent |

**Pale Yellow `#FFFACD` — image tool only.** Do not use in layouts.

### Product color signatures

- **Browserbase** → `#FF4500` Red
- **Stagehand** → `#00C851` Green
- **Director** → `#00B0FF` Blue
...
- `#FF4500` is not a dominant field. One orange element per viewport is almost always right; two is the ceiling.
...
- For product surfaces: use each product's signature color as the accent, not orange.
...
> **Note:** Only Browserbase logo files are currently checked in. Stagehand and Director logos are not yet in `assets/logos/` — request them from the design team before producing Stagehand or Director collateral.
...
| **Logotype** — white | `assets/logos/Browserbase-logo-white.svg` | Same, on dark backgrounds. |
| **Logotype** — black | `assets/logos/browserbase-logo-black.svg` | Same, on light backgrounds without color mark. |
| **Logomark** — color | `assets/logos/Browserbase-logomark-color.svg` | Favicons, avatars, square slots under ~64px, app icons. |
| **Logomark** — white | `assets/logos/Browserbase-logomark-white.svg` | Same, on dark/orange backgrounds. |
| **Logomark** — black | `assets/logos/Browserbase-logomark-black.svg` | Same, on light backgrounds. |
...
- Light background → color (default) or all-black
- Dark background → all-white
- Red/orange background → all-white or all-black (never color-on-color)
...
**Multi-brand ordering:** When showing Browserbase + Stagehand + Director together, order as "bsd."

**Ecosystem lockup types:**
- *Hierarchical* — Browserbase clearly as parent (use when Browserbase is the story)
- *Endorsed* — endorsed brands shown powered-by Browserbase (use for Stagehand/Director contexts)
- *Equal* — peer logos side by side (use for co-branded partner content)
...
| Role | Font | Weight |
|---|---|---|
| **Headlines** | GT Planar | Medium (Browserbase), Regular (Stagehand), Light (Director) |
...
- Headlines use GT Planar Medium (Browserbase), Regular (Stagehand), or Light (Director).
...
**Image themes by brand:**
- **Browserbase only** — Epic Nature, Aggressive Nature, Nostalgic Tech (bold/powerful themes reserved for parent brand)
- **Stagehand** — Builder Tools (precision craftsmanship feel), Emoji-inspired silhouettes
- **Director** — Optimistic Nature, florals, approachable/warm themes. Always use Circle mode.
...
- **Background default**: black or white — not grey, not a color field
- **Accent**: one red (`#FF4500`) element per scene. For Stagehand scenes use green; for Director scenes use blue.
- **Text**: sentence case always. Display sizes use GT Planar Medium (Browserbase), Regular (Stagehand), or Light (Director).
...
### Per-product Remotion defaults

| | Browserbase | Stagehand | Director |
|---|---|---|---|
| Headline font | GT Planar Medium | GT Planar Regular | GT Planar Light |
| Accent color | `#FF4500` Red | `#00C851` Green | `#00B0FF` Blue |
| Imagery tone | Epic/Aggressive Nature | Builder Tools, precision | Optimistic Nature, organic |
| Motion character | Authoritative, stamped | Methodical, precise | Warm, flowing |
...
**Data visualization:**
- **Waffle/dot grid charts** over pie charts — uniform squares or circles on a grid, each colored by brand palette category (red, green, blue, magenta, lime, etc.)
...
- Thin hairline borders (`1px` or `0.5px` at 2x) over heavy separators.
- Corners: subtle rounding (`4px`–`8px`) on cards and inputs; pills (`999px`) only for badges/tags; no hard 16px+ "rounded card" blob aesthetic.
...
- Don't stretch or recolor the logo.
- Don't write marketing-speak ("powerful," "seamless," "next-gen").
- Don't put orange as a full-page background under body copy.
...
2. **Pick a mode.** Light (`#FFFFFF` background, `#000000` text) or dark (`#000000` background, `#FFFFFF` text). State which one you chose in your first reply to the user.
...
6. **Verify.** Before reporting the asset as complete, check: sentence case? One-accent orange? Correct logo variant for the background? No banned colors or fonts?