...
Copy-paste recipes using the `browse` CLI for deterministic UI checks. All commands run against Browserbase cloud browsers.
...
To capture console errors, visit the page and check for JavaScript exceptions:
...
browse eval "
  JSON.stringify({
    ...
    note: 'For real-time capture, inject console override before page load'
  });
"
```
