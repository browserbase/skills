...
description: Use the Browserbase CLI (`bb`) for Browserbase Functions, platform API workflows, and post-session artifact retrieval. Use when the user asks to run `bb`, deploy or invoke Browserbase Functions, create or update sessions, retrieve files downloaded during a remote browser session (`bb sessions downloads get`), upload files to a running session (`bb sessions uploads create`), inspect session logs or rrweb recordings, manage projects, contexts, or extensions, fetch a page through the Browserbase Fetch API, search the web through the Browserbase Search API, or scaffold starter templates. Prefer the Browser skill for live interactive browsing (navigate, click, screenshot, fill forms) — but this skill owns any action on a Browserbase session as a platform resource, even when the session was originally driven by the browse CLI.
