...
- Chrome disposable grouped e2e: PASS on relay ports `19990`, `19997`, `20000`, `20003`, `20004`, `20005`, and `20007`; the latest runtime run includes extension version `0.1.1`, lifecycle/session isolation regression probes, root create/close cleanup with a root `sessionId`, single detach-event assertion, relay CLI screenshot output (`1509933` bytes), and three same-page parallel `fill` + `click #submit` write tasks. Later commits after `d455b35` only update browser-swarm docs and `RUNNING_TEST_NOTES.md`.
