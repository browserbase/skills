...
<html>
  <head><title>arc-serialized-click-test</title></head>
  <body>
    <form id="form">
    ...
    <script>
      document.getElementById("form").addEventListener("submit", (event) => {
        ...
        document.title = "arc-serialized-click-test " + value;
      });
    </script>
  </body>
</html>`;
...
async function main() {
  ...
  if (!allowExistingTargets && health.targetCount !== 0) {
    throw new Error(
      `Refusing to run Arc smoke with ${health.targetCount} existing browser-swarm targets. ` +
  ...
  sessions = targets.map((target, index) =>
    `bs-arcser-${index}-${target.targetId.slice(0, 4).toLowerCase()}`
  );
  const values = ["arc-serialized-alpha", "arc-serialized-beta"];
  ...
  for (let i = 0; i < targets.length; i++) {
    ...
    assert(title === `arc-serialized-click-test ${values[i]}`, `Unexpected title for ${sessions[i]}: ${title}`);
